...
`npm run compliance` requires `@qvac/sdk` and `@qvac/inference` 0.19.0. Judged inference loads `LLAMA_3_2_1B_INST_Q4_0` in-process. Invalid or unavailable QVAC output does not stop Kafka consumption. DHT/provider APIs remain forbidden. After the catalog GGUF is on disk, `fallbackSrc` is that local file. `npm run smoke:offline` was verified on this host: public HTTPS failed, then a live local assessment returned schema-valid `consistent` JSON.
